          ...
          const char *cmd_orig)
{
    ...
    GET_NEXT_TOKEN(cmd_token, cmd_host); //If set udl, cmd_host may be the name of the NIC
                        ...
                        int freshness)
{
    ...
    } else if (strcasecmp(cmd_proto, "udl") == 0) {
        entry->descr.ipproto = CCN_UNDERLAY_PRO;
        socktype = SOCK_RAW;
		goto Underlay;
	...
	return entry;
Underlay:
	entry->descr.address = cmd_host; //the store bit is not set, but I think is OK then.
