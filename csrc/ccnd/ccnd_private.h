...
struct ccn_underlay_faceid_list;
...
struct ccnd_handle {
	...
	unsigned underlay_faceid;		/**< wildcard underlay, bound to port */
	int isunderlay;					/**< if isunderlay==1, then  set underlay socket*/
	char* underlay_eth;				/**< the eth name of underlay*/
	struct ccn_underlay_sock_list *usock_list; /**< linked list to record underlay sock */
	struct ccn_underlay_faceid_list *ufaceid_list;  /**< linked list to record underlay faceid */
	...
	struct ccn_pcap_handle_list *pcap_handle_list;	/* packet capture handle list */
};
...
struct face {
	...
	const struct sockaddr_ll *raw_addr;
    socklen_t addrlen;
	...
	char* eth;
	pcap_t* pcap_handle;
	char* bufferhead;	/**< bufferhead include MAC address and protocol type*/
};
...
#define CCN_FACE_UDL   (1 << 23) /** This face is for underlay */
...
/**
 * Basic data structure to record matches between eth and socket
 */
struct ccn_underlay_sock {
	char* eth;
...
struct ccn_underlay_sock_list {
	...
	struct ccn_underlay_sock_list *next;
};
...
struct ccn_underlay_faceid_list {
	int faceid;
	char* eth;
	struct ccn_underlay_faceid_list *next;
};
...
struct ccnd_handle *ccnd_create(const char *, ccnd_logger, void *, int , char **);
