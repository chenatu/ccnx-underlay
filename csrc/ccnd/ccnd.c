...
use_i:
	...
	ccnd_msg(h, "enroll face %d fd %d", face->faceid, face->recv_fd);
    return (face->faceid);
}
...
init_face_flags(struct ccnd_handle *h, struct face *face, int setflags)
{
	...
	const struct sockaddr_ll *raw_addr = face->raw_addr;
...
/**
 * Set or Create newface for underlay protocol
 */

static struct face *
getNICmatch(struct ccnd_handle *h, int setflags){
	struct hashtb_enumerator ee;
    struct hashtb_enumerator *e = &ee;
    int fd;
    int res;
    struct face *face;
	const int checkflags = CCN_FACE_LINK | CCN_FACE_LOCAL |
                           CCN_FACE_NOSEND | CCN_FACE_UNDECIDED;
	const int wantflags = 0;
	return NULL;
}
...
{
	//ccnd_msg(h,"call check_dgram_faces");
    struct hashtb_enumerator ee;
    struct hashtb_enumerator *e = &ee;
    int count = 0;
                 ...
                 struct ccn_charbuf *reply_body)
{
    ...
    if (face_instance->descr.ipproto != IPPROTO_UDP &&
        ...
        face_instance->descr.ipproto != CCN_UNDERLAY_PRO) {
    ...
    if (face_instance->descr.port == NULL && face_instance->descr.ipproto != CCN_UNDERLAY_PRO) {
        res = ccnd_nack(h, reply_body, 504, "parameter error");
        goto Finish;
    }
	...
	if (face_instance->descr.ipproto == CCN_UNDERLAY_PRO) {
		goto Underlay;
	}
...
Underlay:
	...
	while(ufaceid_list->next != NULL){
		...
		if(strcmp(ueth,ufaceid_list->eth) == 0)
			break;
	}
...
process_incoming_interest(struct ccnd_handle *h, struct face *face,
                          unsigned char *msg, size_t size)
{
	ccnd_msg(h, "process_incoming_interest on face %d fd %d, msg: %s, size: %d", face->faceid, face->recv_fd ,msg,size);
    struct hashtb_enumerator ee;
    struct hashtb_enumerator *e = &ee;
    ...
    if (pi->scope >= 0 && pi->scope < 2 &&
        ...
        if (ie != NULL) {
			...
			ccnd_msg(h, "1 propagate_interest face %d", face->faceid);
            return;
        }
        if (h->debug & 16) {
                ...
                if (k == -1) {
                    ...
                    if (k >= 0) {
                        if (h->debug & (32 | 8))
							...
							ccnd_msg(h, "consume face %d fd %d", face->faceid, face->recv_fd);
                    }
                      ...
                      unsigned char *msg, size_t size, int pdu_ok)
{
	...
	ccnd_msg(h,"process_input_message %s", msg);
...
ccnd_new_face_msg(struct ccnd_handle *h, struct face *face)
{
	...
	if(addr != NULL){
	    if (addr->sa_family == AF_INET6) {
		 ...
		 const struct sockaddr_ll *addrll = raw_addr;
	}
    if (rawaddr != NULL)
...
process_input(struct ccnd_handle *h, int fd)
{
    ...
    if (face == NULL)
        return;
    if ((face->flags & (CCN_FACE_DGRAM | CCN_FACE_UDL | CCN_FACE_PASSIVE)) == CCN_FACE_PASSIVE) {
		//ccnd_msg(h, "listener face %d fd %d", face->faceid, face->recv_fd);
    ...
    if (res == -1)
		...
		if((face->flags & CCN_FACE_UDL) != CCN_FACE_UDL){
			...
			if ((face->flags & CCN_FACE_DGRAM) == 0){
				...
				if (hashtb_seek(e, (struct sockaddr*)(face->raw_addr), rawaddrlen, 0)>=0) {
					source = e->data;
					source->recvcount++;
					if(source->raw_addr== NULL){
						source->raw_addr= e->key;
						source->addrlen = e->keysize;
						source->recv_fd = face->recv_fd;
						source->sendface = face->faceid;
		...
		//ccnd_msg(h,"dres: %d, d->state: %d", dres, d->state); 
        while (d->state == 0) {
...
process_internal_client_buffer(struct ccnd_handle *h)
{
    ...
    if (face->inbuf == NULL)
        return;
	ccnd_msg(h,"inclient buf %s", face->inbuf->buf);
                  ...
                  const void *data, size_t size)
{
    ...
    if (errnum == EAGAIN) {
        ...
        ccnd_msg(h, "send to face %u fd %d failed: %s (errno = %d)",
                 face->faceid, face->recv_fd, strerror(errnum), errnum);
        if (errnum == EISCONN)
            res = 0;
    }
    return(res);
          ...
          const void *data, size_t size)
{
	...
	if ((face->flags & CCN_FACE_UDL) != 0 ){
		ccnd_msg(h, "into (face->flags & CCN_FACE_UDL) != 0");
    ...
    if ((face->flags & CCN_FACE_DGRAM) == CCN_FACE_DGRAM && (face->flags & CCN_FACE_UDL) == 0){
    ...
    if (res == size)
		ccnd_msg(h, "write: %s on fd %d, face %d, len %d", data,face->recv_fd,face->faceid,res);
        return;
    if (res == -1) {
...
/**
 * Create the standard ipv4, ipv6 and underlay bound ports.
 */
static int
		...
		while(usock_list->next != NULL){
			...
			if(raw_fd != -1){
				...
				res = fcntl(fd, F_SETFL, O_NONBLOCK);
	   			if (res == -1)
	        		ccnd_msg(h, "fcntl: %s", strerror(errno));
				res = bind(raw_fd, (struct sockaddr*)raw_addr, sizeof(struct sockaddr_ll));
				if (res != 0) {
					ccnd_msg(h, "socket bind error: %s", strerror(errno));
		        	close(raw_fd);
		        	return(0);
 ...
 */
struct ccnd_handle *
ccnd_create(const char *progname, ccnd_logger logger, void *loggerdata, int argc, char **argv)
{
    char *sockname;
    const char *portstr;
    const char *debugstr;
    ...
    if (h == NULL)
        return(h);
	//initialize the lists, the first one is sentinel
	h->usock_list = (struct ccn_underlay_sock_list*)calloc(1,sizeof(struct ccn_underlay_sock_list));
	h->usock_list->next = NULL;
	h->ufaceid_list = (struct ccn_underlay_faceid_list*)calloc(1,sizeof(struct ccn_underlay_faceid_list));
	h->ufaceid_list->next = NULL;
	h->pcap_handle_list = (struct ccn_pcap_handle_list*)calloc(1,sizeof(struct ccn_pcap_handle_list));
	h->pcap_handle_list->next = NULL;
...
void print_underlay_sock_list(struct ccn_underlay_sock_list *ulist)
{
	...
	while (p->next != NULL){
		p = p->next;
		printf("eth %s, sock %d\n", p->usock.eth, p->usock.sock);
	}
}

void insert_underlay_faceid_list(struct ccn_underlay_faceid_list *ulist, int faceid, char* eth)
{
	struct ccn_underlay_faceid_list * p = ulist;
	while (p->next != NULL)
		p = p->next;
	p->next = (struct ccn_underlay_faceid_list*)calloc(1,sizeof(struct ccn_underlay_faceid_list));
	p = p->next;
	p->faceid = faceid;
	p->eth = eth;
	p->next = NULL;
